...
class Handler
{
     ...
     */
    public function __construct($user_id, string $consumerKey, string $consumerSecret, string $accessToken = null, string $accessTokenSecret = null)
    {
     ...
     * @throws Exception
     */
    public function tweet(string $message, $mediaPath = null, array $media = [], array $options = []): stdClass
    {
        $mediaIds = [];
       ...
       // $media['media_ids'] = ['media_ids' => implode(',', $mediaIds) ?: null];
        return $this->request(
            'tweets',
            'JSONPOST',
            array_filter(array_merge($options, ['text' => $message, 'media'=> $media]), function ($item) {return is_array($item) ? count($item) : strlen($item); })
        );
    }
     ...
     * @throws Exception
     */
    public function follow($user_id): stdClass
    {
        return $this->request('users/:id/following', 'JSONPOST', ['target_user_id' => $user_id]);
    }
     ...
     * @throws Exception
     */
    public function loadUserInfoById($id): stdClass
    {
     ...
     * @throws Exception
     */
    public function loadUserFollowersList(string $username, int $count = 1000, $cursor = null, $cacheExpiry = null): stdClass
    {
        return $this->cachedRequest("users/$username/followers", [
            'max_results' => $count,
    ...
    {
        $res = $this->request("statuses/destroy/$id", 'POST', [], [], 1.1);
        return $res->id ?: false;
    }
    ...
    {
        $res = $this->request("statuses/show/$id", 'GET', [], [], 1.1);
        return $res;
    }
    ...
    {
        $res = $this->request('search/tweets', 'GET', is_array($query) ? $query : ['q' => $query],  [], 1.1);
    ...
    {
        return $this->request("trends/place.json?id=$WOEID", 'GET', [], [], 1.1);
    }
     ...
     * @throws Exception
     */
    public function request(string $resource, string $method, array $data = [], array $files = [], $api_version = 2)
    {
        $api_version = $api_version ?: self::API_Version;
     ...
     * @return stdClass|stdClass[]
     */
    public function cachedRequest(string $resource, array $data = [], $cacheExpire = null, $api_version = 2)
    {
        if (!self::$cacheDir) {
            return $this->request($resource, 'GET', $data, [], $api_version);
        }
        if ($cacheExpire === null) {
